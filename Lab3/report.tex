...
\subsubsection{Вычисление выхода РБФ ячейки}

\begin{lstlisting}[caption={Функция гауссовой активации}]
double gaussian(const Pattern& input, const Pattern& center, double sigma) {
    double distance_sq = 0.0;
    for (int i = 0; i < INPUT_SIZE; i++) {
        double diff = input[i] - center[i];
        distance_sq += diff * diff;
    }
    return exp(-distance_sq / (sigma * sigma));
}
\end{lstlisting}

\subsubsection{Инициализация параметров разброса}

\begin{lstlisting}[caption={Вычисление параметров $\sigma$ для РБФ ячеек}]
void initializeSigmas() {
    for (int i = 0; i < NUM_CLASSES; i++) {
        ...
        // sigma = половина расстояния до ближайшего центра
        sigmas[i] = min_dist / 2.0;
    }
}
\end{lstlisting}

\subsubsection{Обучение выходного слоя}

\begin{lstlisting}[caption={Функция обучения выходного слоя}]
void trainOutputLayer() {
    for (int epoch = 0; epoch < MAX_EPOCHS; epoch++) {
        double max_error = 0.0;
        
        for (int c = 0; c < NUM_CLASSES; c++) {
            // Вычисление выходов РБФ слоя
            vector<double> rbf_outputs(NUM_CLASSES);
            for (int j = 0; j < NUM_CLASSES; j++) {
                rbf_outputs[j] = gaussian(patterns[c], centers[j], sigmas[j]);
            }
            
            // Вычисление выхода сети и коррекция весов
            for (int k = 0; k < NUM_CLASSES; k++) {
                double y_k = 0.0;
                for (int j = 0; j < NUM_CLASSES; j++) {
                    y_k += weights[j][k] * rbf_outputs[j];
                }
                
                double target = (k == c) ? 1.0 : 0.0;
                double error = target - y_k;
                max_error = max(max_error, abs(error));
                
                // Коррекция весов
                for (int j = 0; j < NUM_CLASSES; j++) {
                    weights[j][k] += LEARNING_RATE * error * rbf_outputs[j];
                }
            }
        }
...
\begin{table}[H]
\centering
\caption{Сравнение MLP и RBF сетей}
\small
...
\hline
Параметры & Веса, пороги, $\alpha$ & Центры, $\sigma$, веса \\
\hline
\end{tabular}
\end{table}
